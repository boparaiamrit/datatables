...
class QueryBuilderEngine extends BaseEngine
{
    ...
    public function count()
    {
        ...
        if (!str_contains(mb_strtolower($myQuery->toSql()), ['union', 'having', 'distinct', 'order by', 'group by'])) {
    ...
    protected function compileColumnQuery($query, $method, $parameters, $column, $keyword)
    {
        if (method_exists($query, $method)
            ...
            if (str_contains(mb_strtolower($method), 'raw')
                || str_contains(mb_strtolower($method), 'exists')
            ) {
    ...
    protected function regexColumnSearch($column, $keyword)
    {
        if ($this->isOracleSql()) {
            ...
            $this->query->whereRaw($sql, [mb_strtolower($keyword)]);
        }
    }
